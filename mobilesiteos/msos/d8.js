...
msos.d8 = (function() {
    ...
    function addDays(days) {
        ...
        var targetDate =  msos.d8.create(this.date.getTime() + targetTimeStamp);
        var dstOffset;
    ...
    function getDatesOfCalendarWeek(startWeekOnMonday) {
        ...
        var oneDayInTheFirstWeek = msos.d8.create('01/01/' + year);
        ...
        for (var i = 0; i < 7; i++) {
            ...
            date = msos.d8.create(date.format('mm') + '/' + date.format('dd') + '/' + date.format('yyyy'));
            dates.push(date);
        }
    ...
    function isDST() {
        var y = msos.d8(new Date()).format('yyyy');

        if(new msos.d8('01.01.'+y).date.getTimezoneOffset() == this.date.getTimezoneOffset()) {
            return false;
        } else {
            return true;
        }
...
msos.d8.now = function() {
    var date = new msos.d8();
    date.date = new Date();
    return date;
}

msos.d8.getCalendarWeek = (function(year, month, day) {
    var a = Math.floor((14 - (month)) / 12);
    var y = year + 4800 - a;
    var m = (month) + (12 * a) - 3;
    var jd = day + Math.floor(((153 * m) + 2) / 5) + (365 * y) + Math.floor(y / 4) - Math.floor(y / 100) + Math.floor(y / 400) - 32045;
...
msos.d8.getCalendarWeekByDate = (function(d) {
    return msos.d8.getCalendarWeek(d.format('yyyy'), d.format('mm'), d.format('dd'));
});

msos.d8.getDatesOfCalendarWeek = (function(calendarWeek, startWeekOnMonday, year) {
    var firstDayOfYear = msos.d8.create('01.01.' + year);

    var inLastYear = firstDayOfYear.addDays(-8);

    var cw = msos.d8.getCalendarWeekByDate(inLastYear);

    while(cw !== 1) {
        inLastYear = inLastYear.addDays(1);
        cw = msos.d8.getCalendarWeekByDate(inLastYear);
    }
...
msos.d8.getDateByWeekdayAndCalendarWeek = (function(weekDay, calendarWeek, year) {
...
msos.d8.create = function(dateString) {
...
msos.d8.isLeapYear = (function(year) {
    if(year) {
        year = Number(year);
        if(year) {
            return year % 4 === 0 && year % 100 !== 0 || year % 400 === 0;
...
// Daylight Saving Time aka Summertime
msos.d8.isDST = (function(dateString) {
    var date = msos.d8.create(dateString);
...
msos.d8.version = new msos.set_version(13, 6, 14);